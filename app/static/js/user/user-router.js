...
angular.module('noviga')
  .config(['$routeProvider', function ($routeProvider) {
    $routeProvider
      .when('/managercontrol/users', {
        templateUrl: '/static/views/user/users-mgr.html',
        controller: 'UserMgrController',
        resolve: {
          resolvedAjaxItems: ['$q', '$rootScope', 'binessUserdata', 'Userdata', 'AuthService',
          function ($q, $rootScope, binessUserdata, Userdata, AuthService) {
            var deferred = $q.defer();
            AuthService.getUser()
            .then(
              function(data) {
                ...
                if (data === 'null') {
                  ...
                  if (data.role === 'Admin') {
                    ...
                    binessUserdata.get({businessId: data.businessId}, function(data){
                      deferred.resolve(data);
                    });
                  } else {
                    deferred.reject({authenticated: 'unauthorized'});

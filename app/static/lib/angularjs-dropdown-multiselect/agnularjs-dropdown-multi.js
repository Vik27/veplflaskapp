    ...
    function ($filter, $document, $compile, $parse) {
        ...
        return {
            ...
            template: function (element, attrs) {
                ...
                var template = '<div class="multiselect-parent btn-group btn-group-sm dropdown-multiselect">';
		...
		template += '<li ng-if="(settings.showCheckAll && settings.selectionLimit > 0) || settings.showUncheckAll || settings.showEnableSearchButton" class="divider"></li>';
                ...
                if (checkboxes) {
                    template += '<div class="checkbox"><label><input class="checkboxInput" type="checkbox" ng-click="checkboxClick($event, getPropertyForObject(option,settings.idProp))" ng-checked="isChecked(getPropertyForObject(option,settings.idProp))" /> <span mf-dropdown-static-include="{{settings.template}}"></span></label></div></a>';
                } else {
            ...
            if (angular.isDefined($scope.settings.idProperty)) {
                exists = getIndexByProperty($scope.selectedModel, option, $scope.settings.idProperty) !== -1;
                indexOfOption = getIndexByProperty($scope.selectedModel, option, $scope.settings.idProperty);
            } else {
            ...
            $scope.selectedGroup = null;
                ...
                $scope.isChecked = function (option) {
                    if (angular.isDefined($scope.settings.idProperty)) {
                        return getIndexByProperty($scope.selectedModel, option, $scope.settings.idProperty) !== -1;
                    }
